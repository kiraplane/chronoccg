  ...
  return (
    <div className={cn('flex w-full justify-center', className)}>
      <div style={{ maxWidth: `${ad.width}px`, width: '100%' }}>
        {label ? (
          <div className="mb-2 text-center text-xs text-[#63E6DD]">
            Advertisement
          </div>
        ) : null}
        <iframe
          ...
          title={ad.title}
          className={cn(
            'block max-w-full border-0 bg-[#0A0D10]',
            frameClassName
          )}
        />
      </div>
...
export function AdsterraTopBanner() {
  ...
  return (
    <div className="border-[#3B3128] border-b bg-[#0A0D10] py-2">
...
export function AdsterraAdStack({ className }: { className?: string }) {
  ...
  return (
    <aside className={cn('space-y-8', className)} aria-label="Advertisement">
      <div className="text-center text-xs text-[#63E6DD]">Advertisement</div>
